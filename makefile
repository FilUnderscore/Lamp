...
BOOTLOADER: BOOT_STAGE_1 BOOT_STAGE_2
	...
	dd if=/dev/zero of="bootloader/bin/boot.iso" bs=512 count=720 # Count determines amount of 512b sectors.
	dd if="bootloader/bin/boots1.bin" of="bootloader/bin/boot.iso" seek=0 conv=notrunc
	dd if="bootloader/bin/boots2.bin" of="bootloader/bin/boot.iso" seek=1 conv=notrunc
